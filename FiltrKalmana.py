...
n_max = int(12.5 / dt) + 1
response = []
with open('lab_7_dane/measurements2.txt', 'r') as f:
...
plt.plot(response[0, 0], response[1, 0], 'ro', label='początek trajektorii')
plt.plot(response[0, -1], response[1, -1], 'bo', label='koniec trajektorii')
plt.plot(response[0], response[1], 'gx', label='trajektoria odczytana z pliku')
kfrx, kfry, kfrxs, kfrys = kalman_filter()
plt.plot(kfrx, kfry, 'r', label='trajektoria znaleziona filtrem Kalmana')
plt.plot(kfrxs, kfrys, 'o--', label='estymowana trajektoria')
plt.plot(kfrxs[-1], kfrys[-1], '.', label='położenie samolotu po 5 sekundach')
plt.ylabel('y')
plt.xlabel('x')
plt.legend()
plt.show()
